 ...
 * open at all with no signal. A budget you can't check standing in a shop is a
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) => Promise.all(
      keys.filter((key) => key.startsWith('budget-') && !key.endsWith(CACHE_VERSION))
...
self.addEventListener('fetch', (event) => {
  ...
  if (isDocument) { event.respondWith(shellDocument(request)); return; }
...
async function networkFirst(request) {
  try {
    const response = await fetch(request);
    if (response.ok) { const cache = await caches.open(DATA_CACHE); cache.put(request, response.clone()); }
    return response;
  } catch {
    const cached = await caches.match(request);
    if (cached) { const headers = new Headers(cached.headers); headers.set('X-From-Cache', 'true'); return new Response(await cached.blob(), { status: 200, headers }); }
...
self.addEventListener('push', (event) => {
  ...
  let payload; try { payload = event.data.json(); } catch { payload = { title: 'Family Budget', body: event.data.text() }; }
  event.waitUntil(self.registration.showNotification(payload.title ?? 'Family Budget', { body: payload.body ?? '', icon: './icons/icon-192.png', badge: './icons/icon-192.png', tag: payload.tag ?? 'budget-alert', renotify: true, data: { url: payload.url ?? './index.html' } }));
});
