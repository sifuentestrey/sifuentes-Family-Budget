...
Deno.serve(async (req) => {
  ...
  if (!clientId || !secret) {
    return json({ error: 'not_configured', message: 'PLAID_CLIENT_ID and PLAID_SECRET are not set on this project.' }, 503);
  }
  ...
  try {
    ...
    const exchange = await fetch(`${PLAID_HOST}/item/public_token/exchange`, {
      method: 'POST', headers: plaidHeaders, body: JSON.stringify({ public_token: publicToken }),
    });
    const exchanged = await exchange.json();

    if (!exchange.ok) {
      return json({ error: exchanged.error_code ?? 'exchange_failed', message: exchanged.error_message ?? 'Could not exchange the public token' }, exchange.status);
    }
    ...
    const { error: vaultError } = await admin.rpc('store_vault_secret', { secret_name: tokenRef, secret_value: accessToken });
    ...
    const accountsResponse = await fetch(`${PLAID_HOST}/accounts/get`, {
      method: 'POST', headers: plaidHeaders, body: JSON.stringify({ access_token: accessToken }),
    });
    ...
    return json({ connected: true, institution: institution?.name ?? 'Connected bank', accounts: accountCount, item_id: item.id });
  } catch (error) {
