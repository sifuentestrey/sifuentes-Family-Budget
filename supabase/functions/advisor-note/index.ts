...
async function askGemini(
  ...
  const prompt = [
    ...
    '- Check data_health for stale or missing inputs. Budget allowances are targets, not proof of affordability. Use budget_window and future_paychecks; never reset spending at today.',
    '- A transaction date is cash movement; the linked due date determines which billing month it pays. Never mark a bill paid from amount alone.',
    '- Never invent an amount, due date, merchant, category, paycheck, transaction, or rule.',
    '- Transfers are excluded from spending and income. Help plan bill payments and budget allocations, but never claim to execute payments, transfers or trades.',
