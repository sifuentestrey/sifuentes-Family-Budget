...
test('obvious fast-food merchants are categorized as Dining Out', () => {
  for (const payee of [
    'KFC', 'KFC H287005', "Papa John's", 'PAPA JOHNS #214',
    'Wingstop', 'Jack in the Box', 'Sonic Drive-In', "Mo' Bettahs", 'Queen Donuts',
  ]) {
    assert.deepEqual(
...
test('verified household obligations use deterministic merchant rules', () => {
  const cases = [
    ['Pennymac', 'Rent/Mortgage'],
    ['Advancial Fed Cu DES:Loan Pymt', 'Car Payment'],
    ['7-Eleven', 'Gas'],
    ['Pilot Flying J', 'Gas'],
    ['Texas Health System', 'Medical'],
    ['Namecheap', 'Subscriptions'],
  ];
  for (const [payee, category] of cases) {
    assert.equal(categorizeOne({ payee, plaidCategory: null }, ctx).category, category, payee);
  }
});
